...
module.exports = class Environments extends Diffable {
          ...
          environment.deployment_branch_policy.custom_branch_policies.forEach(policy => {
            if (typeof policy === 'string') {
              ...
              policy.names.forEach(name => {
                policies.push({ name, type: policy.type })
              })
            }
          })
  ...
  async update (existing, attrs) {
    ...
    if (waitTimer || preventSelfReview || reviewers || deploymentBranchPolicy) {
      ...
      const envResult = await this.nopifyRequest('PUT /repos/:org/:repo/environments/:environment_name', options, 'Update environment settings')
    ...
    if (deploymentBranchPolicy && attrs.deployment_branch_policy && attrs.deployment_branch_policy.custom_branch_policies) {
      ...
      for (const policy of existingPolicies) {
        const deleteResult = await this.nopifyRequest('DELETE /repos/:org/:repo/environments/:environment_name/deployment-branch-policies/:branch_policy_id', {
          ...baseRequestOptions,
      ...
      for (const policy of attrs.deployment_branch_policy.custom_branch_policies) {
        const createResult = await this.nopifyRequest(
      ...
      for (const variable of attrs.variables) {
        ...
        if (existingVariable) {
          ...
          if (existingVariable.value !== variable.value) {
            const patchResult = await this.nopifyRequest(
            ...
            if (patchResult) results.push(...patchResult)
          }
        } else {
          const postResult = await this.nopifyRequest(
          ...
          if (postResult) results.push(...postResult)
        }
      }

      for (const variable of existingVariables) {
        const deleteResult = await this.nopifyRequest(
    ...
    if (deploymentProtectionRules) {
      ...
      for (const rule of existingRules) {
        const deleteResult = await this.nopifyRequest('DELETE /repos/:org/:repo/environments/:environment_name/deployment_protection_rules/:rule_id', {
          ...baseRequestOptions,
          rule_id: rule.id
  ...
  async add (attrs) {
    ...
    const envResult = await this.nopifyRequest(
    ...
    if (attrs.deployment_branch_policy && attrs.deployment_branch_policy.custom_branch_policies) {
      for (const policy of attrs.deployment_branch_policy.custom_branch_policies) {
        const policyResult = await this.nopifyRequest(
          'POST /repos/:org/:repo/environments/:environment_name/deployment-branch-policies', {
            ...baseRequestOptions,
            name: policy.name,
            type: policy.type
        ...
        if (policyResult) results.push(...policyResult)
      }
    }

    if (attrs.variables) {
      for (const variable of attrs.variables) {
        const varResult = await this.nopifyRequest(
          'POST /repos/:org/:repo/environments/:environment_name/variables', {
            ...baseRequestOptions,
            name: variable.name,
            value: variable.value
        ...
        if (varResult) results.push(...varResult)
      }
    }

    if (attrs.deployment_protection_rules) {
      for (const rule of attrs.deployment_protection_rules) {
        const ruleResult = await this.nopifyRequest(
          'POST /repos/:org/:repo/environments/:environment_name/deployment_protection_rules', {
            ...baseRequestOptions,
            integration_id: rule.app_id
          }, 'Create deployment protection rule'
